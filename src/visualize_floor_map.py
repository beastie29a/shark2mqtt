...
mpl.use("Agg")  # Use a non-interactive backend for headless rendering


class VisualizeFloorMap:
    ...
    CELL_CATEGORIES: ClassVar[dict[int, int]] = {
        0x00: 0,  # free
        0x01: 1,  # unknown
    ...
    _CELL_LUT: ClassVar[np.ndarray] = np.full(256, len(CELL_COLORS) - 1, dtype=np.uint8)
    ...
    _PX_PER_M: ClassVar[float] = 100.0
    # Physical robot radius, used to size the sprite.
    _ROBOT_RADIUS_M: ClassVar[float] = 0.16

    def __init__(self):
        for _val, _cat in self.CELL_CATEGORIES.items():
            self._CELL_LUT[_val] = _cat

    # ---------------------------------------------------------------------------
    # Protobuf decoding (minimal, self-contained)
    # ---------------------------------------------------------------------------

    def decode_varint(self, buf, offset) -> tuple[int, int]:
        result = 0
        shift = 0
    ...
    def decode_point2d(self, buf) -> tuple[float, float]:
        x = struct.unpack("<f", buf[1:5])[0]
        y = struct.unpack("<f", buf[6:10])[0]
        return (x, y)

    def decode_pose(self, buf) -> tuple[float, float, float] | None:
    ...
    def decode_occupancy_grid(self, buf):
        offset = 0
        grid = {}
        if buf[offset] == 0x0D:
        ...
        if offset < len(buf) and buf[offset] == 0x12:
            ...
            grid["origin"] = self.decode_point2d(origin_data)
            offset += 2 + sub_len
        if offset < len(buf) and buf[offset] == 0x18:
            grid["height"], offset = self.decode_varint(buf, offset + 1)
        if offset < len(buf) and buf[offset] == 0x20:
            grid["width"], offset = self.decode_varint(buf, offset + 1)
        if offset < len(buf) and buf[offset] == 0x28:
            _, offset = self.decode_varint(buf, offset + 1)
        if offset < len(buf) and buf[offset] == 0x32:
            cell_len, offset = self.decode_varint(buf, offset + 1)
            grid["cells"] = buf[offset : offset + cell_len]
            offset += cell_len
        return grid

    def decode_polygon_points(self, buf) -> list[tuple[float, float]]:
        points = []
        i = 0
        while i < len(buf) - 11:
    ...
    def _decode_message(self, buf) -> list[tuple[int, int, bytes | None]]:
        ...
        while offset < len(buf):
            start = offset
            tag, offset = self.decode_varint(buf, offset)
            field_num = tag >> 3
            wire_type = tag & 0x07
            if field_num == 0:
                raise ValueError(f"invalid protobuf tag at offset {start}")
            if wire_type == 0:
                value, offset = self.decode_varint(buf, offset)
                fields.append((field_num, 0, value))
            elif wire_type == 2:
                length, offset = self.decode_varint(buf, offset)
    ...
    def _collect_points(self, sub_msg) -> list[tuple[float, float]]:
        ...
        points = []
        try:
            for field_num, wire_type, payload in self._decode_message(sub_msg):
                ...
                if field_num == 1 and wire_type == 2 and len(payload) >= 10:
                    x, y = self.decode_point2d(payload)
                    points.append((x, y))
        except (ValueError, IndexError, struct.error):
            return []
        return points

    def decode_boundary_payload(self, payload) -> list[tuple[float, float]]:
        ...
        """
        try:
            fields = self._decode_message(payload)
        except ValueError:
            return []
        for field_num, wire_type, field_payload in fields:
            if field_num == 4 and wire_type == 2:
                pts = self._collect_points(field_payload)
                if pts:
                    return pts
        # Fallback: the payload itself is a points message.
        return self._collect_points(payload)

    def _decode_zone_zone_name(self, buf) -> str | None:
        ...
        real_name = None
        try:
            fields = self._decode_message(buf)
        except ValueError:
            fields = []
    ...
    def decode_zone(self, buf) -> dict:
        offset = 0
        zone = {}
        if buf[offset] == 0x08:
            zone["type"], offset = self.decode_varint(buf, offset + 1)
        if offset < len(buf) and buf[offset] == 0x12:
            str_len, offset = self.decode_varint(buf, offset + 1)
        ...
        if offset < len(buf) and buf[offset] == 0x22:
            boundary_len, offset = self.decode_varint(buf, offset + 1)
            boundary_data = buf[offset : offset + boundary_len]
            zone["boundary"] = self.decode_polygon_points(boundary_data)
            offset += boundary_len
        ...
        real_name = self._decode_zone_zone_name(buf)
        if real_name:
            zone["zone_name"] = real_name
        return zone

    async def parse_floor_map(self, filepath) -> dict:
        """Parse the binary into components needed for visualization."""
        data = await asyncio.to_thread(Path(filepath).read_bytes)
        return await self.parse_floor_map_bytes(data)

    async def parse_floor_map_bytes(self, data: bytes) -> dict:
        """async wrapper for parse_floor_map_bytes to avoid blocking the event loop."""
        return await asyncio.to_thread(self._parse_floor_map_bytes, data)

    def _parse_floor_map_bytes(self, data: bytes) -> dict:
        ...
        # Field 2: name
        _, offset = self.decode_varint(data, offset)
        str_len, offset = self.decode_varint(data, offset)
        ...
        _, offset = self.decode_varint(data, offset)
        grid_len, offset = self.decode_varint(data, offset)
        grid_buf = data[offset : offset + grid_len]
        grid_candidates = [self.decode_occupancy_grid(grid_buf)]
        offset += grid_len
        ...
        while offset < len(data):
            tag_val, new_offset = self.decode_varint(data, offset)
            field_num = tag_val >> 3
            ...
            if wire_type == 0:
                ...
                if field_num == 7:
                    pose = self.decode_pose(payload)

                elif field_num == 15:
                    zones.append(self.decode_zone(payload))
                    ...
                    try:
                        grid = self.decode_occupancy_grid(payload)
                        if grid.get("cells"):
                    ...
                    #   32: plain boundaries
                    pts = self.decode_boundary_payload(payload)
                    if pts:
        ...
        result["grid"] = self._pick_grid(grid_candidates, zones)
        result["zones"] = zones
    ...
    def _grid_world_extent(self, grid) -> tuple[float, float, float, float]:
    ...
    def _grid_zone_coverage(self, grid, zones) -> float:
        """Fraction of zone boundary points that fall inside the grid."""
        x_min, x_max, y_min, y_max = self._grid_world_extent(grid)
    ...
    def _pick_grid(self, grid_candidates, zones) -> dict:
        ...
        def score(grid) -> tuple[float, float]:
            coverage = self._grid_zone_coverage(grid, zones)
    ...
    def build_grid_image(self, grid) -> np.ndarray:
    ...
    def _render_floor_map(self, parsed, output_path=None, dpi=150, show_zones=True, show_boundaries=True) -> None:
        ...
        # Build grid image
        img = self.build_grid_image(grid)

        # Create colormap
        cmap = ListedColormap(self.CELL_COLORS)
        norm = BoundaryNorm(range(len(self.CELL_COLORS) + 1), cmap.N)
        ...
        legend_patches = []
        for i, (color, label) in enumerate(zip(self.CELL_COLORS, self.CELL_LABELS)):
        ...
        if show_zones and parsed["zones"]:
            for i, zone in enumerate(parsed["zones"]):
                ...
                legend_patches.append(
                    mpatches.Patch(
                        facecolor=color, alpha=0.3, edgecolor=color, label=f"Zone: {zone.get('zone_name', '')}"
                    )
                )
        if show_boundaries and parsed["boundaries"]:
        ...
        if output_path:
            ...
            logger.info("Saved: %s (%d DPI)", output_path, dpi)
        else:
    ...
    def _render_floor_map_pillow(
        ...
        img_w = max(1, round((x_max - x_min) * self._PX_PER_M))
        img_h = max(1, round((y_max - y_min) * self._PX_PER_M))

        def w2p(x, y) -> tuple[int, int]:
            """World meters (y up) -> image pixels (y down)."""
            return (round((x - x_min) * self._PX_PER_M), round((y_max - y) * self._PX_PER_M))
        ...
        if pose:
            ...
            self._draw_robot(fd, cx, cy, math.degrees(pz), r, outline, fill)

        # Zone labels on top, with a halo for legibility.
        if show_zones:
            font = ImageFont.load_default(size=max(10, round(self._PX_PER_M / 6)))
    ...
    def _draw_robot(self, draw, cx, cy, heading_deg, r, outline, fill) -> None:
